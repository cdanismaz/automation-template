package com.cd.automationtemplate.suites.config;

public class Constants {

    public static class Urls {
        public static final String LOGIN_PAGE = "http://dlpdevelop.vcerf.net";
        public static final String DASHBOARD_PAGE = "http://dlpdevelop.vcerf.net/dashboard/dashboard";
    }

    public static class Credentials {
        public static final String VALID_USERNAME = "admin";
        public static final String VALID_PASSWORD = "admin";

        public static final String INVALID_USERNAME = "test";
        public static final String INVALID_PASSWORD = "test";
    }
}
